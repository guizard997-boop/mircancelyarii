...
def index():
    ...
    cat_chips = ''.join(
        f'<a href="/catalog?category={c.id}" class="px-3 py-1.5 rounded-full text-sm border bg-soft text-brand border-brand/20">{c.name}</a>'
        for c in cats
    )
    content = (
        ...
        '<a href="/catalog" class="btn-o text-white font-bold px-5 py-2.5 rounded-full inline-block mb-6">В каталог</a>'
        f'<div class="flex flex-wrap gap-2 mb-10">{cat_chips}</div>'
        '<h2 class="text-xl font-extrabold mb-4">Новинки</h2>'
        f'<div class="grid grid-cols-2 md:grid-cols-4 gap-4">{cards}</div></div>'
    )
...
def catalog():
    ...
    cats = Category.query.order_by(Category.name).all()
    chip_bits = []
    all_cls = 'bg-brand text-white border-brand' if not cat_id else 'bg-white text-gray-600'
    chip_bits.append(f'<a href="/catalog" class="px-3 py-1.5 rounded-full text-sm border {all_cls}">Все</a>')
    for c in cats:
        active = (cat_id == c.id)
        cls = 'bg-brand text-white border-brand' if active else 'bg-white text-gray-600'
        href = f'/catalog?category={c.id}'
        if q:
            from urllib.parse import quote
            href += '&q=' + quote(q)
        chip_bits.append(f'<a href="{href}" class="px-3 py-1.5 rounded-full text-sm border {cls}">{c.name}</a>')
    chips_html = ''.join(chip_bits)
    ...
    content = (
        ...
        f'<input type="hidden" name="category" value="{cat_val}">'
        f'<input name="q" value="{q_esc}" placeholder="Поиск товаров..." class="flex-1 max-w-md border rounded-full px-4 py-2.5 text-sm">'
        '<button class="btn-g text-white font-bold px-5 py-2 rounded-full text-sm">Найти</button>'
        '</form>'
        f'<div class="flex flex-wrap gap-2 mb-6">{chips_html}</div>'
        f'<div class="grid grid-cols-2 md:grid-cols-4 gap-4">{cards}</div></div>'
    )
...
def format_categories_message():
    cats = Category.query.order_by(Category.name).all()
    if not cats:
        return '📂 Категорий нет. Пришлите 0.', []
    ...
    if st.step == 'wait_category' and text:
        ...
        if t != '0':
            try:
                num = int(t)
                if 1 <= num <= len(cats):
                    cat_id = cats[num - 1].id
                else:
                    tg_send(chat_id, f'Введите число от 0 до {len(cats)}')
                    return 'ok', 200
            except ValueError:
                found = Category.query.filter(Category.name.ilike(f'%{t}%')).first()
                if found:
                    cat_id = found.id
                else:
                    tg_send(chat_id, 'Категория не найдена. Пришлите номер из списка.')
                    return 'ok', 200
        st.draft_category_id = cat_id
        st.step = 'wait_price'
        db.session.commit()
        ...
        if cat_id:
            ...
            if c:
                cat_label = c.name
        tg_send(chat_id, f'Категория: <b>{cat_label}</b>\n\nПришлите <b>цену</b> (число):')
        return 'ok', 200
