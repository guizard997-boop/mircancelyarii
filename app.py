...
SITE_TAGLINE = os.environ.get("SITE_TAGLINE", "Стильные канцтовары для учёбы, работы и твоих больших идей")
...
tailwind.config = { theme: { extend: {
  colors: {
    brand: '#7C3AED', brandd: '#6D28D9', soft: '#FAF5FF', ink: '#2E1065',
    pinky: '#F0ABFC', lilac: '#E9D5FF', accent: '#EC4899'
  },
...
<style>
body{font-family:Manrope,system-ui,sans-serif;background:#FAF5FF;color:#2E1065}
.script{font-family:'Great Vibes',cursive}
.btn{background:linear-gradient(135deg,#8B5CF6,#7C3AED);color:#fff;font-weight:700;border-radius:9999px;padding:.75rem 1.5rem;display:inline-flex;align-items:center;gap:.4rem;box-shadow:0 8px 24px rgba(124,58,237,.25);transition:.15s}
.btn:hover{filter:brightness(1.06);transform:translateY(-1px)}
.btn-pink{background:linear-gradient(135deg,#EC4899,#DB2777);box-shadow:0 8px 24px rgba(236,72,153,.25)}
.card{background:#fff;border-radius:1.25rem;overflow:hidden;border:1px solid #F3E8FF;transition:.2s}
.card:hover{box-shadow:0 16px 40px rgba(124,58,237,.12);transform:translateY(-3px)}
.chip{border-radius:1rem;padding:.5rem .85rem;font-size:.75rem;font-weight:600;background:#fff;border:1px solid #E9D5FF;color:#6D28D9;display:inline-block}
.chip.active{background:linear-gradient(135deg,#8B5CF6,#7C3AED);color:#fff;border-color:transparent}
input,select,textarea{border:1px solid #E9D5FF;border-radius:1rem;padding:.7rem 1rem;width:100%;background:#fff}
input:focus,select:focus,textarea:focus{outline:2px solid #C4B5FD;outline-offset:1px}
.badge-cart{position:absolute;top:-6px;right:-8px;background:#EC4899;color:#fff;font-size:10px;font-weight:800;min-width:18px;height:18px;border-radius:999px;display:flex;align-items:center;justify-content:center;padding:0 5px}
.topbar{background:linear-gradient(90deg,#4C1D95,#6D28D9,#7C3AED);color:#fff;font-size:12px}
.hero-grad{background:linear-gradient(135deg,#F5F3FF 0%,#FAE8FF 45%,#FDF4FF 100%)}
.cat-card{background:#fff;border-radius:1.25rem;padding:1rem;border:1px solid #F3E8FF;text-align:center;transition:.2s}
.cat-card:hover{border-color:#C4B5FD;box-shadow:0 10px 28px rgba(124,58,237,.1)}
.banner-pre{background:linear-gradient(105deg,#6D28D9 0%,#7C3AED 40%,#A855F7 100%);border-radius:1.5rem;color:#fff;overflow:hidden}
.discount{position:absolute;top:10px;left:10px;background:#EC4899;color:#fff;font-size:11px;font-weight:800;padding:3px 8px;border-radius:999px}
.price-old{text-decoration:line-through;color:#A78BFA;font-size:12px}
.bottom-nav{display:none}
@media(max-width:640px){
  .bottom-nav{display:flex;position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:1px solid #F3E8FF;z-index:50;padding:8px 12px calc(8px + env(safe-area-inset-bottom));justify-content:space-around}
...
<div class="topbar">
  <div class="max-w-6xl mx-auto px-4 py-1.5 flex flex-wrap justify-between gap-2 text-[11px] sm:text-xs">
    <span>✦ Предзаказ по Кыргызстану</span>
    <div class="flex gap-3 opacity-90">
      <span>Только предзаказ</span>
      <span>Оплата при получении</span>
      {% if phone %}<a href="tel:{{ phone }}" class="hover:underline">{{ phone }}</a>{% endif %}
    </div>
  </div>
</div>
<header class="bg-white/95 backdrop-blur sticky top-0 z-40 border-b border-purple-100">
  <div class="max-w-6xl mx-auto px-4 h-16 flex items-center gap-3">
    <a href="/" class="flex items-center gap-2 shrink-0">
      <span class="text-2xl">👑</span>
      <div class="leading-tight">
        <div class="font-extrabold text-brandd text-sm sm:text-base tracking-tight">Мир Канцелярии</div>
        <div class="text-[10px] text-purple-400 hidden sm:block">Больше, чем просто канцтовары</div>
      </div>
    </a>
    <nav class="hidden md:flex items-center gap-1 ml-4 text-sm font-semibold text-ink/80">
      <a href="/" class="px-3 py-1.5 rounded-full {% if title=='Главная' %}bg-brand text-white{% else %}hover:bg-purple-50{% endif %}">Главная</a>
      <a href="/catalog" class="px-3 py-1.5 rounded-full hover:bg-purple-50">Каталог</a>
      <a href="/catalog" class="px-3 py-1.5 rounded-full hover:bg-purple-50">Акции</a>
    </nav>
    <form action="/catalog" method="get" class="hidden sm:flex flex-1 max-w-xs ml-auto">
      <input name="q" value="{{ request.args.get('q','') }}" placeholder="Поиск..." class="text-sm !py-2">
    </form>
    <a href="/cart" class="relative ml-2 p-2 rounded-full hover:bg-purple-50 text-brandd font-bold text-sm">
      🛒{% if cart_count %}<span class="badge-cart">{{ cart_count }}</span>{% endif %}
    </a>
    {% if session.get('admin') %}
    <a href="/admin/dashboard" class="text-xs font-bold text-brandd px-2">Админ</a>
    {% endif %}
  </div>
</header>
...
<div class="text-sm px-4 py-2.5 rounded-2xl {% if cat=='success' %}bg-emerald-50 text-emerald-800{% else %}bg-rose-50 text-rose-700{% endif %}">{{ msg }}</div>
{% endfor %}
</div>
{% endif %}
{% endwith %}
...
<footer class="mt-12 bg-white border-t border-purple-100">
  <div class="max-w-6xl mx-auto px-4 py-10 grid sm:grid-cols-4 gap-6 text-sm">
    <div class="sm:col-span-2">
      <div class="flex items-center gap-2 mb-2"><span>👑</span><span class="font-extrabold text-brandd">Мир Канцелярии</span></div>
      <p class="text-purple-800/60 text-xs leading-relaxed">{{ tagline }}</p>
      <p class="script text-2xl text-brand mt-3">Канцелярия которая вдохновляет ♥</p>
    </div>
    <div>
      <div class="font-bold mb-2">Покупателям</div>
      <a href="/catalog" class="block text-purple-800/60 hover:text-brandd text-xs mb-1">Каталог</a>
      <a href="/cart" class="block text-purple-800/60 hover:text-brandd text-xs mb-1">Корзина</a>
    </div>
    <div>
      <div class="font-bold mb-2">Связь</div>
      {% if phone %}<a href="tel:{{ phone }}" class="block text-purple-800/60 text-xs">{{ phone }}</a>{% endif %}
      <p class="text-purple-800/50 text-xs mt-2">Только предзаказ · Без онлайн-доставки на сайте</p>
    </div>
  </div>
  <div class="border-t border-purple-50 py-4">
    <div class="max-w-6xl mx-auto px-4 flex flex-wrap gap-4 justify-center text-[11px] text-purple-800/50 font-semibold">
      <span>🚚 Предзаказ по Кыргызстану</span>
      <span>🛡 Гарантия качества</span>
      <span>♥ Любимые бренды</span>
      <span>👑 Для учёбы и творчества</span>
    </div>
  </div>
</footer>

<nav class="bottom-nav text-[10px] font-bold text-purple-800/70">
  <a href="/" class="flex flex-col items-center gap-0.5"><span class="text-lg">🏠</span>Главная</a>
  <a href="/catalog" class="flex flex-col items-center gap-0.5"><span class="text-lg">📦</span>Каталог</a>
  <a href="/cart" class="flex flex-col items-center gap-0.5 relative"><span class="text-lg">🛒</span>Корзина{% if cart_count %}<span class="badge-cart">{{ cart_count }}</span>{% endif %}</a>
  <a href="/admin" class="flex flex-col items-center gap-0.5"><span class="text-lg">👤</span>Кабинет</a>
</nav>
</body></html>
"""
...
def product_card(p):
    img = p.image_url or "https://placehold.co/400x400/f5f3ff/7c3aed?text=+"
    return f"""
<a href="/product/{p.id}" class="card block group relative">
  <div class="aspect-square bg-purple-50 overflow-hidden relative">
    <img src="{img}" alt="" class="w-full h-full object-cover group-hover:scale-105 transition duration-300" loading="lazy"
      onerror="this.src='https://placehold.co/400x400/f5f3ff/7c3aed?text=+'">
    <span class="absolute top-2 right-2 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center text-purple-300 text-sm">♥</span>
  </div>
  <div class="p-3">
    <div class="font-bold text-sm leading-snug line-clamp-2 min-h-[2.5rem] text-ink">{p.name}</div>
...
def index():
    ...
    # popular category tiles (first 6)
    cat_tiles = ""
    icons = ["✏️", "📓", "🎒", "🎨", "📎", "🏫", "🖊️", "📐"]
    for i, c in enumerate(cats[:6]):
        ic = icons[i % len(icons)]
        cat_tiles += f"""
<a href="/catalog?category={c.id}" class="cat-card">
  <div class="text-3xl mb-2">{ic}</div>
  <div class="text-xs font-bold leading-snug">{c.name}</div>
</a>"""
    cards = "".join(product_card(p) for p in products) or (
        '<p class="col-span-full text-center text-purple-400 py-16">Каталог пока пуст — скоро добавим хиты</p>'
    )
    content = f"""
<section class="hero-grad relative overflow-hidden">
  <div class="max-w-6xl mx-auto px-4 py-10 sm:py-16 grid md:grid-cols-2 gap-8 items-center">
    <div>
      <p class="script text-3xl text-brand mb-1">Твое вдохновение ♥</p>
      <h1 class="text-3xl sm:text-5xl font-extrabold tracking-tight mb-3 text-ink">Мир Канцелярии</h1>
      <p class="text-purple-900/60 mb-6 text-sm sm:text-base leading-relaxed">
        Стильные канцтовары для учёбы, работы и твоих больших идей
      </p>
      <a href="/catalog" class="btn">Перейти в каталог →</a>
      <div class="flex flex-wrap gap-3 mt-6 text-[11px] font-semibold text-purple-800/70">
        <span class="bg-white/80 rounded-full px-3 py-1.5 border border-purple-100">📦 Предзаказ по КР</span>
        <span class="bg-white/80 rounded-full px-3 py-1.5 border border-purple-100">✨ Оригинальная продукция</span>
        <span class="bg-white/80 rounded-full px-3 py-1.5 border border-purple-100">💬 Связь по телефону</span>
      </div>
    </div>
    <div class="hidden md:block relative">
      <div class="rounded-3xl overflow-hidden shadow-2xl shadow-purple-200 border border-purple-100 bg-gradient-to-br from-purple-100 to-pink-100 aspect-[4/3] flex items-center justify-center text-8xl">
        ✏️📓🎀
      </div>
    </div>
  </div>
...
<section class="max-w-6xl mx-auto px-4 py-10">
  <div class="flex items-end justify-between mb-5">
    <h2 class="text-xl font-extrabold">Популярные категории</h2>
    <a href="/catalog" class="text-sm font-bold text-brandd">Смотреть всё →</a>
  </div>
  <div class="grid grid-cols-3 sm:grid-cols-6 gap-3">{cat_tiles or '<p class="text-purple-400 text-sm col-span-full">Категории загрузятся после запуска</p>'}</div>
</section>

<section class="max-w-6xl mx-auto px-4 pb-6">
  <div class="banner-pre p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    <div>
      <div class="text-lg sm:text-xl font-extrabold mb-1">Предзаказ по Кыргызстану</div>
      <p class="text-white/80 text-sm">Любимые товары — даже если их нет в наличии. Оставьте заявку — мы свяжемся.</p>
    </div>
    <a href="/catalog" class="bg-white text-brandd font-extrabold text-sm px-5 py-2.5 rounded-full shrink-0">Узнать подробнее →</a>
  </div>
</section>

<section class="max-w-6xl mx-auto px-4 py-8">
  <div class="flex items-end justify-between mb-5">
    <h2 class="text-xl font-extrabold">🔥 Хиты продаж</h2>
    <a href="/catalog" class="text-sm font-bold text-brandd">Смотреть всё →</a>
  </div>
...
def catalog():
    ...
    cards = "".join(product_card(p) for p in products) or (
        '<p class="col-span-full text-center text-purple-800/50 py-16">Ничего не найдено</p>'
    )
    content = f"""
<div class="max-w-6xl mx-auto px-4 py-8">
  ...
  <p class="text-sm text-purple-800/60 mb-3">{len(products)} товар(ов)</p>
  <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">{cards}</div>
</div>"""
...
<div class="max-w-6xl mx-auto px-4 py-8 grid md:grid-cols-2 gap-8">
  <div class="card aspect-square bg-purple-50">
  ...
  <div>
    ...
    <p class="text-xs text-purple-800/50 mt-4">Только предзаказ · Без доставки · Мы свяжемся с вами</p>
  </div>
</div>"""
...
<div class="max-w-lg mx-auto px-4 py-20 text-center">
  ...
  <p class="text-purple-800/60 mb-6">Добавьте товары из каталога</p>
  <a href="/catalog" class="btn">В каталог</a>
    ...
    for it in items:
        p = it["product"]
        rows += f"""
<div class="flex gap-3 items-center border-b border-purple-50 py-3">
  <img src="{p.image_url or ''}" class="w-16 h-16 rounded-xl object-cover bg-purple-50" onerror="this.style.background='#f0fdfa'">
  <div class="flex-1 min-w-0">
    <a href="/product/{p.id}" class="font-bold text-sm line-clamp-1">{p.name}</a>
    <div class="text-xs text-purple-800/60">{money(p.price)} сом</div>
  </div>
...
<div class="max-w-2xl mx-auto px-4 py-8">
  ...
  <div class="flex items-center justify-between mb-6">
    <span class="text-purple-800/70">Итого</span>
...
<div class="max-w-md mx-auto px-4 py-16 text-center">
  ...
  <p class="text-purple-800/70 mb-6">Мы свяжемся с вами по телефону <b>{phone}</b></p>
  <a href="/catalog" class="btn">В каталог</a>
</div>"""
...
<div class="max-w-md mx-auto px-4 py-8">
  ...
  <p class="text-sm text-purple-800/60 mb-6">Итого: <b class="text-brandd">{money(cart_total())} сом</b> · Без доставки</p>
  <form method="post" class="card p-5 space-y-3">
    <div><label class="text-xs font-bold text-purple-800/60">Имя</label>
    <input name="name" required placeholder="Как к вам обращаться"></div>
    <div><label class="text-xs font-bold text-purple-800/60">Телефон</label>
    <input name="phone" required placeholder="+996 ..." inputmode="tel"></div>
    <div><label class="text-xs font-bold text-purple-800/60">Комментарий</label>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
  <div class="card p-4"><div class="text-xs text-purple-800/50">Товары</div><div class="text-2xl font-extrabold">{pc}</div></div>
  <div class="card p-4"><div class="text-xs text-purple-800/50">Новые заявки</div><div class="text-2xl font-extrabold text-amber-600">{oc}</div></div>
  <div class="card p-4"><div class="text-xs text-purple-800/50">Всего заявок</div><div class="text-2xl font-extrabold">{total_orders}</div></div>
  <div class="card p-4"><div class="text-xs text-purple-800/50">Открытые долги</div><div class="text-2xl font-extrabold text-red-500">{money(debt_open)}</div></div>
</div>
...
<tr class="border-t border-purple-50">
  <td class="py-2 pr-2"><img src="{p.image_url or ''}" class="w-10 h-10 rounded-lg object-cover bg-purple-50"></td>
...
<table class="w-full text-sm"><thead><tr class="text-left text-purple-800/50">
<th class="pb-2"></th><th class="pb-2">Название</th><th class="pb-2">Цена</th><th></th>
</tr></thead><tbody>{rows or '<tr><td colspan="4" class="py-8 text-center text-purple-800/40">Пусто</td></tr>'}</tbody></table>
</div></div>"""
...
<form method="post" enctype="multipart/form-data" class="card p-5 space-y-3">
  ...
  <div><label class="text-xs font-bold text-purple-800/60">Фото</label>
...
def admin_orders():
    ...
    for o in orders:
        items = "".join(
            f'<div class="text-xs text-purple-800/70">• {it.product_name} × {it.quantity} — {money(it.price * it.quantity)} сом</div>'
            for it in o.items
        )
...
<div class="card p-4 mb-3">
  ...
  {f'<div class="text-xs text-purple-800/50 mt-1">{o.comment}</div>' if o.comment else ''}
...
{blocks or '<p class="text-purple-800/40 text-center py-12">Заявок пока нет</p>'}
</div>"""
...
def admin_categories():
    ...
    cats = Category.query.order_by(Category.name).all()
    rows = "".join(
        f'<div class="flex justify-between py-2 border-b border-purple-50 text-sm"><span>{c.name}</span>'
...
<div class="card p-4">{rows or '<p class="text-purple-800/40 text-center">Пусто</p>'}</div>
</div>"""
...
def admin_orgs():
    ...
    for o in orgs:
        ...
        red = "text-red-500" if s else "text-purple-800/30"
        rows += (
            f'<tr class="border-t border-purple-50">'
            f'<td class="py-2 font-semibold text-sm">{o.name}</td>'
            f'<td class="py-2 text-sm text-purple-800/60">{o.phone or "—"}</td>'
...
<p class="text-sm text-purple-800/60 mb-3">Открытых: <b class="text-red-500">{money(total)} сом</b></p>
...
</tr></thead><tbody>{rows or '<tr><td colspan="5" class="py-8 text-center text-purple-800/40">Нет записей</td></tr>'}</tbody></table>
</div></div>"""
...
<div class="max-w-md mx-auto px-4 py-8">
{admin_nav('debts')}
<p class="text-purple-800/60 mb-4">Сначала добавьте организацию.</p>
...
<p class="text-sm text-purple-800/60 mb-4">Товар из каталога → счёт организации</p>
<form method="post" class="card p-5 space-y-3">
  <label class="text-xs font-bold text-purple-800/60">Организация</label>
  <select name="organization_id" required>{o_opts}</select>
  <label class="text-xs font-bold text-purple-800/60">Товар из магазина</label>
  <select name="product_id">{p_opts}</select>
  <label class="text-xs font-bold text-purple-800/60">Количество</label>
  <input type="number" name="quantity" value="1" min="1" max="999">
  <p class="text-xs text-purple-800/50">Или вручную (если товара нет в списке):</p>
