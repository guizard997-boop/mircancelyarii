...
def save_upload(file_storage):
    """Save uploaded file, return /uploads/xxx.ext path or None"""
...
<div><label class="text-sm font-medium">Фото товара</label>
<input type="file" name="photo" accept="image/*" class="w-full border rounded-xl px-4 py-2.5 mt-1 text-sm">
<p class="text-xs text-gray-400 mt-1">Выберите фото с телефона или компьютера (до 8 МБ)</p>
{"<img src=\'"+p.image_url+"\' class=\'mt-2 h-24 rounded-lg object-cover\' onerror=\"this.style.display=\'none\'\">" if p and p.image_url else ""}
</div>
<div><label class="text-sm font-medium text-gray-400">или ссылка на фото (необязательно)</label>
...
def admin_settings():
    if request.method == 'POST':
        ...
        if uploaded:
            set_setting('rekvizity_image', uploaded)
        else:
            url = request.form.get('rekvizity_image', '').strip()
            if url:
                set_setting('rekvizity_image', url)
...
<div>
<label class="text-sm font-medium">Фото / QR реквизитов</label>
<input type="file" name="rekvizity_photo" accept="image/*" class="w-full border rounded-xl px-4 py-2.5 mt-1 text-sm">
<p class="text-xs text-gray-400 mt-1">Загрузите фото с телефона</p>
{"<img src='"+rekvizity_image+"' class='mt-3 max-h-48 rounded-xl border' onerror=\"this.style.display='none'\">" if rekvizity_image else ""}
</div>
<div>
<label class="text-sm font-medium text-gray-400">или ссылка (необязательно)</label>
<input name="rekvizity_image" value="" class="w-full border rounded-xl px-4 py-2.5 mt-1" placeholder="https://...">
</div>
<button class="bg-primary text-white font-semibold px-6 py-2.5 rounded-xl">Сохранить</button>
</form></div>"""
